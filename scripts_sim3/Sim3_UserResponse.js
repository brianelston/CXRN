...
//Patient's laryngeal edema corrected but the dose is greater than initially recommended
	...
	SetPatientResponseColorRed();
				//Completion of scenario although the dose is greater than initially recommended
	alert("Epinephine 0.5 mg or more IV fluid. \n\nCompletion of scenario although the dose is greater than initially recommended.");
	
	//in 15 secs show report
	//StartShowReportTimer(15000);
	StartShowReportTimer(5000); //bfe 8.21.12 - 5 secs now
}
...
function PerformEPI_IV_pt1mg_LessState(LocalMachine)
{
	AppendtoActionDisplay(TotalEPI_IV + " mg Epiniphrine in IV. No change in vitals.");
...
function InterpretFlushIV(LocalMachine)
{
	...
	switch(LASTIVMedAdministered)
	{
		...
		case StateNameListEnum.EPI_IV_pt1mg_Less:				//done, epi in IV and flushed
				PerformEPI_IV_pt1mg_LessState(LocalMachine);
		break;
	}
...
function InterpretAdministerMedication(LocalMachine)
{
	...
	switch(meditem.text)	//these are the displayed text in the html page
	{
		...
		case "Epinephrine IV":	//.1 mg is correct, or 1  mL
			//we store in Mg - changed 6.1.11
			if(doseunit.value == "mg (milligrams)")
			{
				TotalEPI_IV = (parseFloat(TotalEPI_IV) + parseFloat(meddosenumber))
				TotalEPI_IV = TotalEPI_IV.toPrecision(2);
			}
			else if(doseunit.value == "mL (milliliters)")
			{
				TotalEPI_IV = (parseFloat(TotalEPI_IV) + (parseFloat(meddosenumber)/10.0))
				TotalEPI_IV = TotalEPI_IV.toPrecision(2);
			}
			else
			{
			...
			//alert(" TotalEPI_IV = " + TotalEPI_IV);
			//return;
			
			if(TotalEPI_IV < 0.099)//0.1)			
			{	
				if(StateItemList[StateNameListEnum.IVFluid].StateVisited == true)
				{
					PerformEPI_IV_pt1mg_LessState(LocalMachine);	//if IV fluid flowing, dont need to flush
					return;
				}
				//wait till flush to do
				LASTIVMedAdministered=StateNameListEnum.EPI_IV_pt1mg_Less;	//need to flush to admin, store out drug in IV
				AppendtoActionDisplay("Epinephrine in IV.");
				return;
			}
			else if(TotalEPI_IV >= 0.5)
			{	//incorrect solution
				//needs a flush
			...
			else //if(TotalEPI_IV > 0.09 && TotalEPI_IV < 0.11)//.1 mg
			{
				...
				else
				{
					...
					PlayDingSfx();
					AppendtoActionDisplay(TotalEPI_IV + " mg Epiniphrine in IV. Change in vitals.");
